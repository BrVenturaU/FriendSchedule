...
public class AmigoService implements IAmigoService {
    ...
    public ArrayList<Amigo> getAllFavorites(Context context){
        //Conección con la base de datos
        contextSqLiteHelper = new DbContextSqLiteHelper(context);
        db = contextSqLiteHelper.getWritableDatabase();
        ...
        try{
            //Consultamos los datos (Select)
            Cursor cursor = db.query(FeedDataContract.AmigoEntry.TABLE_NAME, campos, FeedDataContract.AmigoEntry.COLUMN_ES_FAVORITO + " = 1", null,
                    null, null, null);
            if(cursor != null){
                Integer  id = 0;
                String nombre = "";
